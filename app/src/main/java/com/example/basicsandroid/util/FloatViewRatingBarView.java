...
public class FloatViewRatingBarView extends View {
    ...
    public FloatViewRatingBarView(Context mContext, AttributeSet attrs) {
        super(mContext, attrs);
        init(mContext, attrs);
    }

    public FloatViewRatingBarView(Context mContext, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context mContext, AttributeSet attrs) {
        ...
        starNum = typedArray.getInteger(R.styleable.star_starsNum, 5);//获得星星的数量
        starHeight = (int) typedArray.getDimension(R.styleable.star_starHeight, 0);//星星高度
        starWidth = (int) typedArray.getDimension(R.styleable.star_starWidth, 0);//星星宽度
        starDistance = (int) typedArray.getDimension(R.styleable.star_starDistance, 0);//星星间距
        isClick = typedArray.getBoolean(R.styleable.star_starClickable, true);//能否点击  默认不可点击
        starBackgroundBitmap = typedArray.getDrawable(R.styleable.star_starBackground);//星星背景
        starDrawDrawable = drawableToBitmap(typedArray.getDrawable(R.styleable.star_starDrawBackground));//需要变化的背景
